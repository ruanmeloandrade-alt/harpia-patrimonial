...
- A branch continua avançando em paralelo; commits recentes observados são majoritariamente F5/Inbox e não substituem os syncs F2 listados abaixo.
- Pendências identificadas pela F02: tipos Supabase centrais desatualizados; hardening do `public-lead-ingest` para não confiar em `metadata.clientId`; idempotência do favorites store versus RLS sem UPDATE; sincronização dos hardenings F2 recentes.
...
- Hardening de contato: `5f8f1f3984619b930750bcf301a1d0e30da0a9cd` — exige nome + WhatsApp antes de chamar backend e pré-preenche dados conhecidos.
...
- Comparação recente F01 × F02 mostra deltas funcionais em `PublicExperience.tsx`, `usePublicFavoritesBridge.ts`, `useClientAreaData.ts`, `Front02IntegrationShell.tsx` e `catalogQuery.ts`; precisam ser sincronizados para o produto integrado.
...
- A versão atual da F3 avançou: catálogo público filtra unidades sem empreendimento publicado e `getByIdOrCode` compara código sem diferenciar maiúsculas/minúsculas.
...
- A branch F04 continua avançando em Inbox/automação/runtime compartilhado, sem alteração que elimine a pendência de integridade do `clientId` do lead público.
...
- Necessidade: sincronizar os hardenings funcionais recentes da F2: `PublicExperience.tsx`, `usePublicFavoritesBridge.ts`, `useClientAreaData.ts`, `Front02IntegrationShell.tsx` e `catalogQuery.ts`.
- Motivo: correções de contato obrigatório, troca de sessão, estabilidade de favoritos, isolamento de conta cliente e sanitização de filtros.
...
- sincronizar os cinco arquivos funcionais F2 recentes para a branch integrada;
...
- rota de imóvel com percent-encoding inválido ainda precisa de hardening coordenado em `PublicSiteApp.tsx`.
