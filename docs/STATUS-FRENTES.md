...
- Últimos hardenings relevantes:
  ...
  - favoritos por ID antes do slug, proteção entre sessões e serialização contra clique duplo;
  - área do cliente ignora resposta obsoleta após troca de conta;
  - shell limita dados pessoais a conta `client` ativa;
  - filtros URL sanitizados e faixa de preço invertida normalizada;
...
- Comparação atual F01 × F02 mostra **10 arquivos funcionais** com delta a sincronizar:
...
- Último avanço observado após isso foi de handoff/documentação, sem quebra de contrato.
- Ainda precisa sincronização F3 → F1 para a integração usar a versão mais nova do producer.
...
- Avanços recentes observados são internos de fila/QA e não quebram o contrato público.
...
- Sincronizar os 10 arquivos funcionais listados acima.
...
- Sincronizar a versão atual do `PublicCatalogService`/runtime do catálogo para a integração global.
- Urgência: ALTA antes do QA de catálogo público com dados reais.
- Status: PENDENTE DE SYNC.
...
- sincronizar os 10 arquivos funcionais F2;
- sincronizar producer/runtime atual da F3;
