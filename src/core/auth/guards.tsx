...
export function InternalRoute({
  ...
  const required = permissions?.length ? permissions : permission ? [permission] : [];
  const allowed = required.length === 0
    || (requireAll ? required.every(auth.hasPermission) : required.some(auth.hasPermission));
  if (!allowed) return <FullPageState eyebrow="SEM PERMISSÃO" title="Você não pode acessar este módulo" description="Solicite ao administrador uma permissão adicional para esta função." actionHref="/interno" actionLabel="Voltar ao painel" />;
  return <>{children}</>;
}
