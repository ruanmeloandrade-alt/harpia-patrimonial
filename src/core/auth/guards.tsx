...
export function ClientRoute({ children }: PropsWithChildren) {
  ...
  if (auth.profile && !auth.profile.is_active) return <FullPageState eyebrow="CONTA INATIVA" title="Seu acesso está temporariamente indisponível" description="Entre em contato com a Hárpia para verificar sua conta." actionHref="/" actionLabel="Voltar ao site" />;
  return <>{children}</>;
}
