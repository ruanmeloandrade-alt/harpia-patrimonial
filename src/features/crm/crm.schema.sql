...
create table public.crm_lead_custom_field_values (
  lead_id uuid not null references public.crm_leads(id) on delete cascade,
  field_id uuid not null references public.crm_custom_field_definitions(id) on delete restrict,
  value jsonb,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
create trigger crm_stages_touch_updated_at
...
create trigger crm_tags_touch_updated_at
before update on public.crm_tags
for each row execute function private.touch_updated_at();

create trigger crm_custom_field_definitions_touch_updated_at
before update on public.crm_custom_field_definitions
for each row execute function private.touch_updated_at();

create trigger crm_lead_custom_field_values_touch_updated_at
before update on public.crm_lead_custom_field_values
for each row execute function private.touch_updated_at();
...
create trigger inbox_conversations_touch_updated_at
...
create policy "crm_pipelines_select_authorized"
...
create policy "crm_stages_select_authorized"
...
create policy "crm_leads_select_authorized"
...
create policy "crm_tags_select_authorized"
...
create policy "crm_lead_tags_select_authorized"
on public.crm_lead_tags for select to authenticated
using (
  private.user_has_permission((select auth.uid()), 'crm.view')
  or private.user_has_permission((select auth.uid()), 'crm.manage')
  or private.user_has_permission((select auth.uid()), 'inbox.view')
...
create policy "crm_custom_fields_select_authorized"
on public.crm_custom_field_definitions for select to authenticated
using (
  private.user_has_permission((select auth.uid()), 'crm.view')
  or private.user_has_permission((select auth.uid()), 'crm.manage')
  or private.user_has_permission((select auth.uid()), 'inbox.view')
...
create policy "crm_custom_values_select_authorized"
on public.crm_lead_custom_field_values for select to authenticated
using (
  private.user_has_permission((select auth.uid()), 'crm.view')
  or private.user_has_permission((select auth.uid()), 'crm.manage')
  or private.user_has_permission((select auth.uid()), 'inbox.view')
...
create policy "crm_tasks_select_authorized"
...
create policy "crm_history_select_authorized"
...
create policy "crm_pipelines_insert_manage"
on public.crm_pipelines for insert to authenticated
with check (private.user_has_permission((select auth.uid()), 'crm.manage'));
create policy "crm_pipelines_update_manage"
on public.crm_pipelines for update to authenticated
using (private.user_has_permission((select auth.uid()), 'crm.manage'))
with check (private.user_has_permission((select auth.uid()), 'crm.manage'));

create policy "crm_stages_insert_manage"
on public.crm_stages for insert to authenticated
with check (private.user_has_permission((select auth.uid()), 'crm.manage'));
create policy "crm_stages_update_manage"
on public.crm_stages for update to authenticated
using (private.user_has_permission((select auth.uid()), 'crm.manage'))
with check (private.user_has_permission((select auth.uid()), 'crm.manage'));
create policy "crm_stages_delete_manage"
...
create policy "crm_tags_insert_manage"
on public.crm_tags for insert to authenticated
with check (private.user_has_permission((select auth.uid()), 'crm.manage'));
create policy "crm_tags_update_manage"
on public.crm_tags for update to authenticated
using (private.user_has_permission((select auth.uid()), 'crm.manage'))
with check (private.user_has_permission((select auth.uid()), 'crm.manage'));
create policy "crm_tags_delete_manage"
on public.crm_tags for delete to authenticated
using (private.user_has_permission((select auth.uid()), 'crm.manage'));

create policy "crm_lead_tags_insert_manage"
on public.crm_lead_tags for insert to authenticated
with check (private.user_has_permission((select auth.uid()), 'crm.manage'));
create policy "crm_lead_tags_delete_manage"
on public.crm_lead_tags for delete to authenticated
using (private.user_has_permission((select auth.uid()), 'crm.manage'));

create policy "crm_custom_fields_insert_manage"
on public.crm_custom_field_definitions for insert to authenticated
with check (private.user_has_permission((select auth.uid()), 'crm.manage'));
create policy "crm_custom_fields_update_manage"
on public.crm_custom_field_definitions for update to authenticated
using (private.user_has_permission((select auth.uid()), 'crm.manage'))
with check (private.user_has_permission((select auth.uid()), 'crm.manage'));
create policy "crm_custom_fields_delete_manage"
on public.crm_custom_field_definitions for delete to authenticated
using (private.user_has_permission((select auth.uid()), 'crm.manage'));

create policy "crm_custom_values_insert_manage"
on public.crm_lead_custom_field_values for insert to authenticated
with check (private.user_has_permission((select auth.uid()), 'crm.manage'));
create policy "crm_custom_values_update_manage"
on public.crm_lead_custom_field_values for update to authenticated
using (private.user_has_permission((select auth.uid()), 'crm.manage'))
with check (private.user_has_permission((select auth.uid()), 'crm.manage'));
create policy "crm_custom_values_delete_manage"
on public.crm_lead_custom_field_values for delete to authenticated
using (private.user_has_permission((select auth.uid()), 'crm.manage'));

create policy "crm_tasks_insert_manage"
on public.crm_tasks for insert to authenticated
with check (private.user_has_permission((select auth.uid()), 'crm.manage'));
create policy "crm_tasks_update_manage"
on public.crm_tasks for update to authenticated
using (private.user_has_permission((select auth.uid()), 'crm.manage'))
with check (private.user_has_permission((select auth.uid()), 'crm.manage'));
create policy "crm_tasks_delete_manage"
on public.crm_tasks for delete to authenticated
using (private.user_has_permission((select auth.uid()), 'crm.manage'));

create policy "crm_history_insert_manage"
on public.crm_lead_history for insert to authenticated
with check (private.user_has_permission((select auth.uid()), 'crm.manage'));

-- Inbox: leitura exige inbox.view ou inbox.manage.
create policy "inbox_conversations_select_authorized"
...
create policy "inbox_messages_select_authorized"
...
create policy "inbox_conversations_insert_manage"
