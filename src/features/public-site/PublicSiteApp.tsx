    ...
    <div className="harpia-public">
      <header className="public-header">
        <a className="brand" href="/" onClick={intercept('/')} aria-label="Hárpia Patrimonial - início">
          ...
          <span><strong>HÁRPIA</strong><small>PATRIMONIAL & CO.</small></span>
        </a>
      ...
      <footer className="public-footer">
        <div><strong>HÁRPIA PATRIMONIAL & CO.</strong><p>Inteligência patrimonial especializada em negócios imobiliários.</p></div>
...
function HomePage({
  ...
  loading: boolean;
  error: string;
  filterOptions: PublicCatalogFilterOptions;
    ...
    <main>
      <section className="hero section-shell">
        <div className="hero-copy">
          ...
          <p>A Hárpia orienta negócios imobiliários, investimentos e decisões patrimoniais com uma visão que vai além da transação.</p>
        ...
        <aside className="hero-search" aria-label="Busca de imóveis">
          <HomeCatalogSearch options={filterOptions} onNavigate={onNavigate} />
        </aside>
      </section>
      ...
      <section className="section-shell">
        <div className="section-heading">
          <div><p className="section-kicker">Catálogo</p><h2>Imóveis e oportunidades publicados</h2></div>
      ...
      <section className="lifestyle-section section-shell">
        <div className="section-heading">
          <div><p className="section-kicker">Encontre pelo estilo de vida</p><h2>O imóvel certo também depende de como você quer viver.</h2></div>
        </div>
        {filterOptions.lifestyleTags.length === 0 ? (
          ...
          <div className="lifestyle-tags">
            {filterOptions.lifestyleTags.map((tag) => (
              <button key={tag} type="button" onClick={() => onNavigate(`/imoveis?estilo=${encodeURIComponent(tag)}`)}>{tag}</button>
            ))}
          </div>
        )}
      </section>
      ...
      <section className="service-grid section-shell">
        {[
          ...
          <article className="service-card" key={title}>
            <p className="section-kicker">Serviço</p><h3>{title}</h3><p>{copy}</p>
      ...
      <section className="duo-section section-shell">
        <article>
          <p className="section-kicker">DUMU Arquitetura</p><h2>Patrimônio, espaço e bem viver.</h2>
        ...
        <article>
          <p className="section-kicker">Atendimento consultivo</p><h2>Não é só encontrar um imóvel. É entender a decisão.</h2>
...
function CatalogPage({ items, loading, error, filters, options, onFilters, onOpen, onFavorite, isFavorite }: {
  items: PublicCatalogItem[];
  loading: boolean;
  error: string;
  ...
  return (
    <main className="section-shell catalog-page">
      ...
      <div className="filter-grid">
        <label>Finalidade<select value={filters.purpose ?? ''} onChange={(event) => onFilters({ ...filters, purpose: event.target.value || undefined })}><option value="">Todas</option>{options.purposes.map((value) => <option key={value}>{value}</option>)}</select></label>
        <label>Cidade<select value={filters.city ?? ''} onChange={(event) => onFilters({ ...filters, city: event.target.value || undefined, location: undefined })}><option value="">Todas</option>{options.cities.map((value) => <option key={value}>{value}</option>)}</select></label>
        <label>Localização<select value={filters.location ?? ''} onChange={(event) => onFilters({ ...filters, location: event.target.value || undefined })}><option value="">Todas</option>{options.locations.map((value) => <option key={value}>{value}</option>)}</select></label>
        <label>Lançamento<select value={filters.launch === undefined ? '' : String(filters.launch)} onChange={(event) => onFilters({ ...filters, launch: event.target.value === '' ? undefined : event.target.value === 'true' })}><option value="">Todos</option><option value="true">Sim</option><option value="false">Não</option></select></label>
        <label>Preço mínimo<input type="number" min="0" inputMode="numeric" value={filters.minPrice ?? ''} placeholder={options.minPrice === null ? 'Sem mínimo' : formatCurrency(options.minPrice)} onChange={(event) => onFilters({ ...filters, minPrice: event.target.value ? Number(event.target.value) : undefined })} /></label>
        <label>Preço máximo<input type="number" min="0" inputMode="numeric" value={filters.maxPrice ?? ''} placeholder={options.maxPrice === null ? 'Sem máximo' : formatCurrency(options.maxPrice)} onChange={(event) => onFilters({ ...filters, maxPrice: event.target.value ? Number(event.target.value) : undefined })} /></label>
        {options.lifestyleTags.length > 0 && <label>Estilo de vida<select value={filters.lifestyleTag ?? ''} onChange={(event) => onFilters({ ...filters, lifestyleTag: event.target.value || undefined })}><option value="">Todos</option>{options.lifestyleTags.map((value) => <option key={value}>{value}</option>)}</select></label>}
      </div>
      <div className="catalog-toolbar" aria-live="polite">
        ...
        {hasCatalogFilters(filters) && <button className="text-button" type="button" onClick={() => onFilters({})}>Limpar filtros</button>}
      </div>
        ...
        return (
          <article className="property-card" key={item.id}>
            <div className="property-media">
              ...
              <button className="favorite-button" type="button" aria-pressed={saved} aria-label={saved ? 'Remover dos favoritos' : 'Salvar imóvel'} onClick={() => onFavorite(item)}>{saved ? '♥' : '♡'}</button>
            </div>
            <div className="property-body">
              <small>{item.propertyType} · {item.purpose}</small><h3>{item.title}</h3><p>{item.location}, {item.city}</p><strong>{formatCurrency(item.price)}</strong>
              <div className="property-meta">{item.privateAreaM2 ? <span>{item.privateAreaM2} m²</span> : null}{item.bedrooms ? <span>{item.bedrooms} quartos</span> : null}{item.parkingSpaces ? <span>{item.parkingSpaces} vagas</span> : null}<span>{item.status === 'sold' ? 'Vendido' : 'Publicado'}</span></div>
  ...
  return (
    <main className="section-shell property-detail">
      ...
      <div className="detail-hero">
        <div className="detail-copy"><p className="section-kicker">{item.propertyType} · {item.purpose}</p><h1>{item.title}</h1><p>{item.location}, {item.city}</p><strong className="detail-price">{formatCurrency(item.price)}</strong><div className="hero-actions"><button className="primary-button" type="button" onClick={() => void onService(item)}>Quero atendimento</button><button className="secondary-button" type="button" aria-pressed={isFavorite(item.id)} onClick={() => onFavorite(item)}>{isFavorite(item.id) ? 'Remover dos favoritos' : 'Salvar imóvel'}</button></div></div>
        <div className="detail-cover">{images[0] ? <img src={images[0].url} alt={images[0].alt ?? item.title} /> : <span>Imagem não disponível</span>}</div>
      </div>
      <div className="detail-grid">
        <article><p className="section-kicker">Sobre o imóvel</p><h2>Características</h2><p>{item.description || 'Descrição ainda não cadastrada no catálogo publicado.'}</p><div className="feature-list">{item.features?.length ? item.features.map((feature) => <span key={feature}>{feature}</span>) : <span>Características detalhadas ainda não cadastradas.</span>}</div></article>
        <aside className="detail-facts"><strong>Informações</strong>{item.privateAreaM2 ? <span>Área privativa: {item.privateAreaM2} m²</span> : null}{item.bedrooms ? <span>Quartos: {item.bedrooms}</span> : null}{item.suites ? <span>Suítes: {item.suites}</span> : null}{item.bathrooms ? <span>Banheiros: {item.bathrooms}</span> : null}{item.parkingSpaces ? <span>Vagas: {item.parkingSpaces}</span> : null}<span>Status: {item.status === 'sold' ? 'Vendido' : 'Publicado'}</span>{item.development ? <span>Empreendimento: {item.development.title}{item.development.unitLabel ? ` · ${item.development.unitLabel}` : ''}</span> : null}</aside>
      </div>
      {images.length > 1 && <div className="detail-gallery">{images.slice(1).map((image) => <img loading="lazy" key={image.url} src={image.url} alt={image.alt ?? item.title} />)}</div>}
      {videos.length > 0 && <div className="video-list">{videos.map((video) => <a key={video.url} href={video.url} target="_blank" rel="noreferrer">Abrir vídeo do imóvel</a>)}</div>}
    </main>
  );
}
...
function OwnerCapture({ intent, onSubmit }: {
  intent: OwnerIntent;
  onSubmit: (payload: { intent: OwnerIntent; city: string; propertyType: string; contact: { name: string; email: string; whatsapp: string } }) => Promise<boolean>;
}) {
  ...
  return (
    <main className="owner-page section-shell">
      <div className="page-intro"><p className="section-kicker">Captação de proprietário</p><h1>Quero {intent === 'vender' ? 'vender' : 'alugar'} meu imóvel</h1><p className="section-lead">Envie os dados iniciais. Nesta primeira versão, a equipe Hárpia conduz o processo manualmente.</p></div>
      <form className="owner-form" onSubmit={handleSubmit} aria-busy={status === 'busy'}>
        ...
        <button className="primary-button" type="submit" disabled={status === 'busy'}>{status === 'busy' ? 'Enviando…' : 'Enviar solicitação'}</button>
        {feedback ? <p className={status === 'error' ? 'form-feedback form-feedback--error' : 'form-feedback'} role={status === 'error' ? 'alert' : 'status'}>{feedback}</p> : null}
      </form>
    </main>
  );
}
