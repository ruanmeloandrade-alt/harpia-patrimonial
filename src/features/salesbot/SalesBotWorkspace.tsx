...
export function SalesBotWorkspace({ canManage = false }: { canManage?: boolean }) {
  ...
  const importFile = async (file: File) => {
    ...
    try {
      ...
      setError(errorMessage(error, 'Não foi possível importar este SalesBot.'));
    }
  };

  const exportBot = (bot: SalesBotDefinition) => {
    const blob = new Blob([JSON.stringify({ format: 'harpia-salesbot', version: 1, exportedAt: new Date().toISOString(), ...bot }, null, 2)], { type: 'application/json' });
