...
export function PermissionsPage() {
  ...
  async function saveGroupEdit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!editingGroup || editingGroup.is_system || !canManage) return;
    const form = new FormData(event.currentTarget);
    ...
    try {
      ...
      if (selectedGroup?.id === editingGroup.id) {
        setSelectedGroup({ ...selectedGroup, name, description: description || null });
      }
      setEditingGroup(null);
      setNotice('Grupo atualizado.');
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Não foi possível editar o grupo.');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function openGroup(group: PermissionGroup) {
    ...
    setError(null);
    try {
      setSelectedPermissions(await getGroupPermissionIds(group.id));
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Não foi possível carregar as permissões do grupo.');
    } finally {
  ...
  async function saveGroupPermissions() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Não foi possível atualizar as permissões do grupo.');
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="workspace-page">
      ...
      {showForm ? <form className="panel form-panel" onSubmit={submit}>
        <div className="form-grid">
          <label className="field"><span>Nome do grupo</span><input name="name" required /></label>
          <label className="field"><span>Identificador</span><input name="slug" placeholder="ex.: corretores" pattern="[a-z0-9-]+" required /></label>
          <label className="field field-wide"><span>Descrição</span><textarea name="description" rows={3} /></label>
        </div>
        <div className="form-actions"><button className="button button-primary" disabled={saving}>{saving ? 'Criando...' : 'Criar grupo'}</button></div>
      </form> : null}

      {editingGroup ? <form className="panel form-panel" onSubmit={saveGroupEdit}>
        <div className="section-heading"><div><p className="eyebrow dark">EDITAR GRUPO</p><h2>{editingGroup.name}</h2><p className="muted">O identificador técnico permanece estável; nome e descrição podem ser ajustados sem quebrar referências.</p></div><button type="button" className="text-button" onClick={() => setEditingGroup(null)}>Cancelar</button></div>
        <div className="form-grid">
          <label className="field"><span>Nome do grupo</span><input name="name" defaultValue={editingGroup.name} required /></label>
          <label className="field"><span>Identificador</span><input value={editingGroup.slug} disabled /></label>
          <label className="field field-wide"><span>Descrição</span><textarea name="description" rows={3} defaultValue={editingGroup.description || ''} /></label>
        </div>
