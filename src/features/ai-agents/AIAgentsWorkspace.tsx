...
export function AIAgentsWorkspace({ canManage = false }: { canManage?: boolean }) {
  ...
  return <section className="f05-module">
    ...
    <fieldset className="f05-readonly-fieldset" disabled={!canManage}><div className="f05-create-row"><input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Nome interno do agente"/><button disabled={!newName.trim()} onClick={() => { try { const agent = createAIAgent(newName); setNewName(''); setError(''); refresh(agent.id); } catch (error) { setError(errorMessage(error, 'Não foi possível criar o agente IA.')); } }}>Criar agente</button></div></fieldset>
    {error && <div className="f05-alert">{error}</div>}
    <div className="f05-split">
      <aside className="f05-list">{agents.length === 0 ? <div className="f05-empty">Nenhum agente criado.</div> : agents.map((agent) => <button key={agent.id} className={`f05-list-item ${agent.id === selectedId ? 'is-active' : ''}`} onClick={() => { setSelectedId(agent.id); setError(''); }}><strong>{agent.name}</strong><span>{agent.status}{agent.role ? ` · ${agent.role}` : ''}</span></button>)}</aside>
      <fieldset className="f05-editor f05-readonly-fieldset" disabled={!canManage}>{!selected ? <div className="f05-empty f05-empty--large">Crie ou selecione um agente.</div> : <>
        ...
        <div className="f05-actions"><button disabled={selected.status !== 'active' && !canActivate} onClick={() => { try { setAIAgentStatus(selected.id, selected.status === 'active' ? 'paused' : 'active'); setError(''); refresh(selected.id); } catch (error) { setError(errorMessage(error, 'Não foi possível alterar o status do agente IA.')); } }}>{selected.status === 'active' ? 'Pausar' : 'Ativar'}</button><button className="danger" onClick={() => { if (!window.confirm('Excluir este agente?')) return; try { deleteAIAgent(selected.id); setError(''); refresh(); } catch (error) { setError(errorMessage(error, 'Não foi possível excluir o agente IA.')); } }}>Excluir</button></div>
