...
import { addAutomationAction, createAutomation, deleteAutomation, listAutomations, removeAutomationAction, setAutomationStatus, updateAutomation, updateAutomationAction } from './repository';
...
const EVENTS: Array<{ value: CrmAutomationEventType; label: string }> = [
  ...
  { value: 'task.due', label: 'Tarefa/data atingida' }, { value: 'custom.event', label: 'Evento customizado' },
];
  ...
  return <section className="f05-module">
    ...
    <div className="f05-split">
      ...
      <div className="f05-editor">{!selected ? <div className="f05-empty f05-empty--large">Crie ou selecione uma automação.</div> : <>
        ...
        <div className="f05-palette"><h3>Condições opcionais</h3>{selected.trigger.conditions.map((condition, index) => <div className="f05-condition-row" key={`${index}-${condition.field}`}><input value={condition.field} placeholder="Campo/evento" onChange={(e) => { const conditions = [...selected.trigger.conditions]; conditions[index] = { ...condition, field: e.target.value }; patch({ trigger: { ...selected.trigger, conditions } }); }}/><select value={condition.operator} onChange={(e) => { const conditions = [...selected.trigger.conditions]; conditions[index] = { ...condition, operator: e.target.value as typeof condition.operator }; patch({ trigger: { ...selected.trigger, conditions } }); }}><option value="equals">igual</option><option value="not_equals">diferente</option><option value="contains">contém</option><option value="exists">existe</option></select><input value={condition.value ?? ''} placeholder="Valor" onChange={(e) => { const conditions = [...selected.trigger.conditions]; conditions[index] = { ...condition, value: e.target.value }; patch({ trigger: { ...selected.trigger, conditions } }); }}/><button className="icon danger" onClick={() => patch({ trigger: { ...selected.trigger, conditions: selected.trigger.conditions.filter((_, i) => i !== index) } })}>×</button></div>)}<button className="secondary" onClick={() => patch({ trigger: { ...selected.trigger, conditions: [...selected.trigger.conditions, { field: '', operator: 'equals', value: '' }] } })}>Adicionar condição</button></div>
